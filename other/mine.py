...
class DQN():
        ...
        def create_Q_network(self):
                # network weights
                W1 = self.weight_variable([self.state_dim, 10])
                b1 = self.bias_variable([10])
                W2 = self.weight_variable([10, 5])
                b2 = self.bias_variable([5])

                W3 =  self.weight_variable([5, self.action_dim])
                b3 = self.bias_variable([self.action_dim])

                #input layer
                self.state_input = tf.placeholder("float", [None, self.state_dim])
                # hidden layers
                h1_layer = tf.matmul(self.state_input, W1) + b1
                ...
                if episode %100 == 0:
                        ...
                        for i in xrange(TEST):
                                ...
                                for j in xrange(STEP):
                                        ...
                                        state, reward, done, ob = env.step(action)
                                        total_reward += reward
